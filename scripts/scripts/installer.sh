...
sudo pacman --color always --force --noconfirm -S ranger ttf-dejavu awesome-terminal-fonts bspwm sxhkd termite polybar feh compton ranger htop scrot atom terminus-font thunar dunst redshift
# dmenu replaced with patched dmenu2 in AUR
sudo systemctl enable gdm.service

# Automatically DL + install all programs from the AUR; this will take a while.
yaourt --color --force --noconfirm -S w3m google-chrome oh-my-zsh spotify imagemagick hsetroot neofetch gtop rofi-dmenu dunstify wal-git i3lock-color-git grub-customizer nerd-fonts-complete pacmanity
